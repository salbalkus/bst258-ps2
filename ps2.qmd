...
Conditional exchangeability implies that the mean of $Y^a$ is the same in both the pseudo-population and the regular population (which we prove in question 2). This further implies that, in the pseudo-population, (a) unconditional exchangeability (no confounding) holds, (b) $E(Y^a)  = E(Y|A = a)$ and (c) association indicates causation.
...
prop_model <- glm(qsmk ~ sex + age + race + education + smokeintensity + 
                    smokeyrs + active + exercise + wt71 + age^2 + wt71^2 + 
                    smokeintensity^2 + smokeyrs^2, data = df, 
...
# Note that the 1 - g(L) weights are already 
# included in the ipw_c_weight variable
